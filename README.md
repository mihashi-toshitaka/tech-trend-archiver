...
request_time_utc: {window_end_iso}
window_start_utc: {window_start_iso}（この時刻を含む）
window_end_utc: {window_end_iso}（この時刻を含まない）
...
検索ツール合計10回以内はSystemプロンプト上の目安であり、リクエスト側では総呼び出し回数を固定していません。Responses APIではinline citationが既定で有効なため、`include` は指定していません。
...
curl -X POST https://api.x.ai/v1/responses \
  ...
  -d '{
    ...
    "input": [
      ...
      {
        "role": "user",
        "content": "以下の対象期間について調査してください。\n\nrequest_time_utc: 2026-07-11T03:00:00.000Z\nwindow_start_utc: 2026-07-10T03:00:00.000Z（この時刻を含む）\nwindow_end_utc: 2026-07-11T03:00:00.000Z（この時刻を含まない）\ntime_zone: Asia/Tokyo\n\n検索ツールが対象期間外の候補を返した場合は除外してください。"
      }
    ],
    "tools": [
      {
