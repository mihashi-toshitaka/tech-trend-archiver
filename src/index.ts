...
const PROMPT = `ここ24時間のX(旧：Twitter)での、IT技術についての話題を収集して、ホットな話題になっているIT技術情報（アフェリエイト目的関係除く）について、以下のフォーマットで返却してください。
情報の記述形式は、シンプルな「見出し＋簡潔な内容」、での箇条書きでお願いします。
最新脆弱性情報については、CNA・NVDの情報を中心とし、ゼロデイ脆弱性や重大な脆弱性情報が話題になっている場合はそれも含めてください。
重要度の高いものは太字で強調してください。
